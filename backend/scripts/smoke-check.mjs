...
const initialMigration = readIfExists('prisma/migrations/20260101000000_init/migration.sql');
const brandToolMigration = readIfExists('prisma/migrations/20260508_add_brand_tool_access/migration.sql');
const rbacMigration = readIfExists('prisma/migrations/20260509_add_rbac_onboarding/migration.sql');
const accountLifecycleMigration = readIfExists('prisma/migrations/20260509_add_account_lifecycle/migration.sql');
const b2bB2cMigration = readIfExists('prisma/migrations/20260514_add_b2b_b2c_rbac_audit/migration.sql');
...
check('brand tool access migration exists', brandToolMigration.includes('brand_tool_access') || initialMigration.includes('"brand_tool_access"'));
...
check('legacy migration backfills existing brands', (brandToolMigration + schemaSql).includes("'legacy'") && (brandToolMigration + schemaSql).includes('ON CONFLICT (brand_id, tool_id) DO NOTHING'));
...
check('rbac migration exists and backfills memberships', (rbacMigration.includes('CREATE TABLE IF NOT EXISTS app_users') || initialMigration.includes('CREATE TABLE "app_users"')) && (rbacMigration + schemaSql).includes('brand_memberships') && (rbacMigration + initialMigration + schemaSql).includes('owner_user_id'));
...
check('account lifecycle sql and migration exist', schemaSql.includes('CREATE TYPE app_user_status') && schemaSql.includes('CREATE TABLE IF NOT EXISTS team_invitations') && ((accountLifecycleMigration.includes('ADD COLUMN IF NOT EXISTS status') && accountLifecycleMigration.includes('CREATE TABLE IF NOT EXISTS team_invitations')) || (initialMigration.includes('"app_user_status"') && initialMigration.includes('CREATE TABLE "team_invitations"'))));
